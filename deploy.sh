#!/usr/bin/env bash
# Canonical deploy script — see rules/docker.md § Deploy and rollback.
#
# Copy verbatim into a project's root as ./deploy.sh, chmod +x, and change only
# the four marked lines below. Everything else is identical in every project.
#
# Deploy <name>, keeping every previous image tagged so a rollback needs no
# build and no network. See README.md § Deploy and rollback.
set -euo pipefail

cd "$(dirname "$0")"

IMAGE_NAME="${IMAGE_NAME:-ghcr.io/cheminfo/tex.cheminfo.org}" # CHANGE — must match compose*.yaml
SERVICE="tex"                                 # CHANGE — the service built from this repo
...
healthy() {
  ...
  port="${port:-10422}" # CHANGE — the project's derived port
  docker compose ps --status running --quiet | grep -q . || return 1
  local i=0
  while [ "$i" -lt 30 ]; do
    # CHANGE — the health path: /v1/health for a backend, / for a static site.
    if docker compose exec -T "$SERVICE" wget -q -O /dev/null "http://127.0.0.1:${port}/v1/health" >/dev/null 2>&1; then
      return 0
    fi
    sleep 2
    i=$((i + 1))
